...
public class AddToCartSD {
    ...
    public void userAddsToTheCart(int quantity, String productName) throws InterruptedException {
    ...
    public void userShouldSeeInTheCart(int quantity, String pdtName) {
        By pdtNameFld = By.xpath("//a[normalize-space()='Blue Shoes']");
        By pdtQuantity = By.cssSelector("input[type=\"number\"]");
        String actualPdtName = driver.findElement(pdtNameFld).getText();
        String actualPdtQuantity = driver.findElement(pdtQuantity).getAttribute("value");
        Assert.assertEquals(pdtName, actualPdtName);
        Assert.assertEquals(quantity, Integer.parseInt(actualPdtQuantity));
    }
}
